...
export function CylinderCarousel({ containerRef }: CylinderCarouselProps) {
  ...
  // Build fix: unused variable removed
  ...
  const [isVisible, setIsVisible] = useState(false); // Initially hidden, waits for scroll

  useEffect(() => {
    // If containerRef passed but null, wait
    if (!actualContainerRef.current || !canvasRef.current) return;

    // We no longer create ScrollSmoother here.
      ...
      img.onload = () => {
        ...
        if (loadedImages === numImages) {
          ...
          ScrollTrigger.create({
            ...
            start: "top bottom", // when the top of the container hits the bottom of the viewport (starts coming into view) 
            end: "bottom top",
            onEnter: () => setIsVisible(true),
            onLeaveBack: () => setIsVisible(false) // Hide if we scroll back up completely past it (optional, maybe we want it visible)
            // Actually, if we are in variant 2, we want this hidden.
            // V2 occupies first 900vh. V1 occupies next 500vh.
            // "top bottom" means this section is about to enter. 
          });
    ...
    return () => {
      ...
      // smoother.kill(); // Removed
    };
  }, [actualContainerRef]);

  return (
    <div style={{ opacity: isVisible ? 1 : 0, transition: 'opacity 0.5s', pointerEvents: isVisible ? 'auto' : 'none' }}>
      ...
      */}
      ...
      <div className="fixed bottom-8 right-8 z-10 pointer-events-none">
        <div className="flex flex-col items-center gap-2 animate-bounce">
          ...
          <span className="text-sm font-mono text-white/40">EXPLORE V</span>
        </div>
      </div>

      {/* Scroll wrappers removed */}
    </div>
  );
}
